...
def do_the_mlp(
    ...
    Parameters:
       input_shape_for_mlp: shape of the input aem.
    ...
    if input_shape_for_mlp is None:
        raise CNNException
    ...
    # generate the input
    input_layer = tf.keras.Input(shape=input_shape_for_mlp)
...
def train_and_predict_for_classification(
    ...
    # generate the model
    mlp_model = do_the_mlp(
        input_shape_for_mlp=input_shape_for_mlp,
    ...
    for i, (train_index, test_index) in enumerate(selected_cs.split(y)):
        ...
        score = mlp_model.evaluate(X_validation, y_validation)[0]
        prediction = mlp_model.predict(X_validation)
