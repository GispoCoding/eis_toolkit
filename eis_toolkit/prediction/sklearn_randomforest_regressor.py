...
def sklearn_randomforest_regressor(
    ...
    Args:
       ...
       - random_state: RandomState instance or None, default=None: Controls both the randomness of the bootstrapping of the samples used when building trees (if bootstrap=True) and the sampling of the features to consider when looking for the best split at each node (if max_features < n_features). See Glossary for details.
       - verbose (default=0): Controls the verbosity when fitting and predicting.
       - warm_start (default=False): When set to True, reuse the solution of the previous call to fit and add more estimators to the ensemble, otherwise, just fit a whole new forest. See the Glossary.
