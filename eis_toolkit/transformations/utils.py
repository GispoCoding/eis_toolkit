...
def expand_args(selection: List[int | str], **kwargs) -> dict:  # might be deleted
...
def replace_nan(  # can be deleted
    ...
    if set_nan == True and not np.isnan(nodata_value):
        out_array[out_array == nodata_value] = np.nan
    elif set_value == True and not np.isnan(nodata_value):
        out_array[np.isnan(out_array)] = nodata_value

    return out_array


def read_raster(  # can be deleted
...
def select_columns(  # can be deleted
    ...
    if columns_geometry:
        columns_no_transform.remove(columns_geometry)
...
def df_to_input_ordered_array(  # can be deleted
    in_data: Union[pd.DataFrame, gpd.GeoDataFrame],
    columns: Optional[List[str]] = None,
