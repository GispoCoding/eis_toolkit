...
def sklearn_model_crossvalidation(  # type: ignore[no-any-unimported]
    ...
    """
        Perform cross validation for a Sklearn model.

    Args:
        - sklearnMl (model). The Model will be fitted based on the training dataset.
          Even for comparison with a testset (or verification dataset) the model is used to get the model-typ
        ...
        - cv (int, default = 4). cross-validation generator or an iterable,
            ...
            In all other cases, Fold is used. These splitters are instantiated with shuffle=False so the
        ...
        - pre_dispatch (int or str, default= 2*n_jobs):
            Controls the number of jobs that get dispatched during parallel execution. Reducing this number can
            be useful to avoid an explosion of memory consumption when more jobs get dispatched than CPUs can process.
        ...
        - return_train_score: Computing training scores is used to get insights on how different parameter
          settings impact the overfitting/underfitting trade-off. However computing the scores on the training
          set can be computationally expensive and is not strictly required to select the parameters that yield
    ...
    Returns:
        Dictionary of the reached scores
            ...
            - test_score
                The score array for test scores on each cv split. Suffix _score in test_score changes to a specific
                metric like test_r2 or test_auc if there are multiple scoring metrics in the scoring parameter.
            - train_score
                The score array for train scores on each cv split. Suffix _score in train_score changes to a specific
                metric like train_r2 or train_auc if there are multiple scoring metrics in the scoring parameter.
