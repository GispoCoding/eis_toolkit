...
def _mlp_regressor(  # type: ignore[no-any-unimported]
    ...
    myML = MLPRegressor()
                ...
                #     (hidden_layer_sizes = self.configuration['hidden_layer_sizes'], \
                #  activation = self.configuration['activation'], \
                #  solver = self.configuration['solver'], \
                #  max_iter = int(self.configuration['max_iter']))
    # \
   ...
   # wenn ydf nicht angegeben istr: aus Xdf trennen
    if ydf is None:
      if fields is None:
...
solver = Literal["lbfgs", "sgd", "adam"]
learning = Literal["constant", "invscaling", "adaptive"]
def mlp_regressor(  # type: ignore[no-any-unimported]
    Xdf: pd.DataFrame,      # dataframe of Features for traning
    ...
    """ training of a multilayer percetron (MLP) model
