...
# 7.2 import csv
name_csvp = str(parent_dir.joinpath(r"data/csv/Test_Test.csv"))

fields, df, urdf, metadata = import_featureclass(fields=myFieldsp, file=name_csvp, decimalpoint_german=True)
...
parent_dir = Path(__file__).parent.__str__() + "/data/Ergebnis"
name_fc = "csv_Ergebnis"
ext = ".csv"
Ergebnis = export_featureclass(
    ydf=ydf1,
    dfg=urdf,
    outpath=parent_dir,
    outfile=name_fc,
...
ext = "csv"
Ergebnis_proba = export_featureclass(
    ydf=ydf2,
    outpath=parent_dir,
    outfile=name_fc,
    outextension=ext,
    decimalpoint_german=True,
)  # nanmask = nodatmask,decimalpoint_german=True,)  #,outextension = ext,, decimalpoint_german=True
...
validation, confusion, comparison, sklearnMl = sklearn_model_validations(
    sklearnMlp, ydf=yt, predict_ydf=ydf1, comparison=True
)  # , fields = columns) #, test_size = 0.2)

filesdict = export_files(
    name="test_rwe_datenverglich",
    path=parent_dir,
    validations=validation,
    confusion_matrix=confusion,
    comparison=comparison,
