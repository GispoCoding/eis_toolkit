...
class NNFactory:
    ...
    def generate_dynamically_the_MLP(self, input_channel_1: tuple, rescaling: bool ,dense_nodes: list, final_output: int, last_activation: str, l_2: float):
        """ """
...
class MLP(NNFactory):
    ...
    def _make_the_one_hot_encoding(self, labels_to_transform):
        try:
            """
    ...
    def compute_MLP_workload(self, data_class_0, data_class_1, label_class_0, label_class_1, write_report="report.csv",
        ...
        """
        WHAT THIS FUNCTION DO + ALL PARAMS EXPLAINED
        ...
        try:
            ...
            for round_number in range(number_of_total_combination + 1):
                ...
                labels = self._make_the_one_hot_encoding(labels_to_transform=labels)
                ...
                for hidden_layer in list_of_hidden_layers:
                    for l2_current_values in np.logspace(-1, 1, num=10, base=10):
                        ...
                        print(f'[CURRENT RUN] Hidden layer {hidden_layer} and L2 is {l2_current_values}')
                        ...
                        for ii, (train_index, test_index) in enumerate(loo.split(data)):
                            ...
                            model = self.generate_dynamically_the_MLP(input_channel_1=(X_train.shape[1], X_train.shape[2]),
