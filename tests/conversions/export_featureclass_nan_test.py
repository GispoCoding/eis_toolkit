...
name_csv = str(parent_dir.joinpath(r"data/csv/Test_Test_kl.csv"))
...
Xvdf_test, Xcdf_test, ydf_dmp, igdf_test = separation(df=df, fields=myFieldsp)
# nodata_replacement of
...
def test_export_featureclass():
    ...
    ydf_prd1 = export_featureclass(
        ydf=ydfpr, outpath=path, outfile="csv_tst_ohneXohneNAN", outextension="csv", decimalpoint_german=True
    )
    ydf_prd1 = export_featureclass(
        ydf=ydfpr,
        nanmask=nanmask,
        outpath=path,
        outfile="csv_tst_ohneXmitNAN",
        outextension="csv",
        decimalpoint_german=True,
    )
    ydf_prd2 = export_featureclass(
        ydf=ydfpr, dfg=Xdf_pr, outpath=path, outfile="csv_tst_mitXohnNAN", outextension="csv", decimalpoint_german=True
    )
    ydf_prd3 = export_featureclass(
        ydf=ydfpr,
        dfg=urdf,
        nanmask=nanmask,
        outpath=path,
        outfile="csv_tst_mitXmitNAN",
        outextension="csv",
        decimalpoint_german=True,
    )
