...
def test_crs():
    ...
    cba_grid.init_from_vector_file(
        cell_size=4000, vector_file_path=vector_path, target_attribut="Litho", subset_of_target_attribut_values="all"
    )
    shp_cba_grid = CBA()
...
def test_add_points_buffered():
    ...
    cba_grid.add_layer(
        vector_file_path=points_path, target_attribut="", subset_of_target_attribut_values=None, Name="Occ", buffer=4000
    )
    shp_cba_grid = CBA()
    shp_cba_grid = shp_cba_grid.from_vector_file(matrix_path)
