...
def test_lee_additive_multiplicative_noise_filter():
    """Test the focal filter function."""
    with rasterio.open(raster_path_single) as raster:
        result, _ = lee_additive_multiplicative_noise_filter(raster, size=5)
...
def test_number_bands():
    ...
    with rasterio.open(raster_path_multi) as raster:
        with pytest.raises(InvalidRasterBandException):
            lee_additive_multiplicative_noise_filter(raster)
...
def test_window_size():
    ...
    with rasterio.open(raster_path_single) as raster:
        with pytest.raises(InvalidParameterValueException):
            # Too small
            lee_additive_multiplicative_noise_filter(raster, size=0)

            # Even number
            lee_additive_multiplicative_noise_filter(raster, size=4)
