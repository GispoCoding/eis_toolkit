...
# 7.2 import fc
name_fcp = name_fc
columns, df, urdf, metadata = import_featureclass(
    fields=myFieldsp, file=name_fcp
)  # , layer = layer_name  ) #, decimalpoint_german = True)
...
Ergebnis = export_featureclass(
    ydf=ydf1, dfg=urdf, metadata=metadata, outpath=parent_dir, outfile=name_fc, nanmask=nodatmask
)
...
ext = ""
# name_fc = parent_dir.joinpath(r'data/Ergebnis.shp')
# Ergebnis_predict = export_featureclass(ydf = ydf2, metadata = metadata, outpath = parent_dir, outfile = name_fc)
