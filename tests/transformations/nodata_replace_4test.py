...
def test_nodata_replace_error():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        nodata_replace(df=ydf, rtype="wrong")
    with pytest.raises(BeartypeCallHintParamViolation):
        nodata_replace(df=fields_csv, rtype="replace")
    with pytest.raises(BeartypeCallHintParamViolation):
        nodata_replace(df=Xvdf, rtype="n_neighbors", n_neighbors="2")
    with pytest.raises(BeartypeCallHintParamViolation):
        nodata_replace(df=Xvdf, replacement_number="1", replacement_string=2, n_neighbors="2")
