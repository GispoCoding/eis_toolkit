...
# df, nodatmask = nodata_remove(df = df,)   # for images
...
def test_separation():
    ...
    if (Xvdf is not None) and (ydf is not None):
        assert len(Xvdf.index) == len(ydf.index)
    if (Xcdf is not None) and (ydf is not None):
...
def test_separation_error():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        separation(df=df, fields="eins:3")
    with pytest.raises(BeartypeCallHintParamViolation):
        separation(df=columns, fields=columns)
