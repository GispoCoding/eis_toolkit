...
Xdfneu, ohe = onehotencoder(df = Xcdf,)
...
cv = sklearn_model_crossvalidation (sklearnMl = sklearnMl, Xdf = train_X, ydf = train_y, cv = 3,)
# cv:  Crossvalidation (dictionary)
print ('cv')

# 5.5 Export of the validation results 
path = str(parent_dir.joinpath(r'data'))   
filesdict = export_files(name = rname,
       path = path,
...
sklearnMl = sklearn_model_fit (sklearnMl = sklearnMl, Xdf = Xdf, ydf = ydf,) # fields = fields,)
