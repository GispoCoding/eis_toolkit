...
from eis_toolkit.exceptions import InvalidParameterValueException   #NonMatchingCrsException, NotApplicableGeometryTypeException
...
def test_sklearn_randomforest_classifier_wrong():
    ...
    with pytest.raises(InvalidParameterValueException):
        sklearnML = sklearn_randomforest_classifier(ccp_alpha = -2.0)
    with pytest.raises(BeartypeCallHintParamViolation):
        sklearnML = sklearn_randomforest_classifier(criterion = 'ich')
    with pytest.raises(BeartypeCallHintParamViolation):
        sklearnML = sklearn_randomforest_classifier(n_estimators = 10.1)
    with pytest.raises(BeartypeCallHintParamViolation):
        sklearnML = sklearn_randomforest_classifier(max_depth = ['1'])
    with pytest.raises(BeartypeCallHintParamViolation):
        sklearnML = sklearn_randomforest_classifier(max_features = ['1'])
    with pytest.raises(BeartypeCallHintParamViolation):
        sklearnML = sklearn_randomforest_classifier(class_weight = 0)
