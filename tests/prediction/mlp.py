...
def test_MLP_classifier():
    """Test that MLP classifier works as expected."""
    metrics = ["accuracy", "precision", "recall"]
    model, out_metrics = train_MLP_classifier(X_IRIS, Y_IRIS, metrics=metrics)
...
def test_invalid_learning_rate():
    ...
    with pytest.raises(exceptions.InvalidParameterValueException):
        train_MLP_classifier(X_IRIS, Y_IRIS, learning_rate=-1)
    with pytest.raises(exceptions.InvalidParameterValueException):
        train_MLP_regressor(X_IRIS, Y_IRIS, learning_rate=-1)
