...
def test_train_and_evaluate_with_no_validation():
    ...
    model, out_metrics = _train_and_evaluate_sklearn_model(
        X, y, model=RF_MODEL, test_method="none", metrics=CLF_METRICS, random_state=42
    )
...
def test_train_and_evaluate_with_simple_split():
    ...
    model, out_metrics = _train_and_evaluate_sklearn_model(
        X, y, model=RF_MODEL, test_method="simple_split", metrics=CLF_METRICS, simple_split_size=0.25, random_state=42
    )
...
def test_train_and_evaluate_with_kfold_cv():
    ...
    model, out_metrics = _train_and_evaluate_sklearn_model(
        X, y, model=RF_MODEL, test_method="kfold_cv", metrics=CLF_METRICS, cv_folds=3, random_state=42
    )
...
def test_train_and_evaluate_with_skfold_cv():
    ...
    model, out_metrics = _train_and_evaluate_sklearn_model(
        X, y, model=RF_MODEL, test_method="skfold_cv", metrics=CLF_METRICS, cv_folds=3, random_state=42
    )
...
def test_mismatching_X_and_y():
    ...
    with pytest.raises(exceptions.NonMatchingParameterLengthsException):
        _train_and_evaluate_sklearn_model(X, y[:-1], model=RF_MODEL, test_method="none", metrics=CLF_METRICS)
...
def test_invalid_metrics():
    ...
    with pytest.raises(exceptions.InvalidParameterValueException):
        _train_and_evaluate_sklearn_model(X, y, model=RF_MODEL, test_method="simple_split", metrics=[])


def test_invalid_cv_folds():
    """Test that invalid metric selection raises the correct exception."""
    with pytest.raises(exceptions.InvalidParameterValueException):
        _train_and_evaluate_sklearn_model(X, y, model=RF_MODEL, test_method="kfold_cv", metrics=CLF_METRICS, cv_folds=1)
...
def test_invalid_simple_split_size():
    ...
    with pytest.raises(exceptions.InvalidParameterValueException):
        _train_and_evaluate_sklearn_model(
            X, y, model=RF_MODEL, test_method="simple_split", metrics=CLF_METRICS, simple_split_size=0.0
        )
